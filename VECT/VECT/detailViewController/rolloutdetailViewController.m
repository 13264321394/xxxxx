...
@interface rolloutdetailViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    if (StatusbarHeight==44) {
        ...
        self.tableView=[[UITableView alloc]initWithFrame:CGRectMake(0,StatusbarHeight+44+10,GZDeviceWidth,GZDeviceHeight-StatusbarHeight-10-44) style:UITableViewStylePlain];
    }
...
-(void)getMoveDataSource
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.001 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        NSURL *url = [NSURL URLWithString:[portUrl stringByAppendingString:@"/appUser/outDetails"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        cell.iconImageView.image=[UIImage imageNamed:@"VECToutput.png"];
        
        NSString *thestr=[NSString stringWithFormat:@"%@", [self.dataSource[indexPath.row] objectForKey:@"stis"]];
        
        if (thestr.length==0) {
            cell.label1.text=@"空";
        } else {
            cell.label1.text=thestr;
        }
        cell.label1.font=[UIFont systemFontOfSize:3.5+GZDeviceWidth/35];
        cell.label1.textColor = [UIColor blackColor];
        cell.label1.lineBreakMode= NSLineBreakByTruncatingMiddle;
        ...
        float floatstr=[[self.dataSource[indexPath.row] objectForKey:@"cashNumber"] floatValue];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    moreoutViewController *mdv=[[moreoutViewController alloc] init];
    mdv.str1=[NSString stringWithFormat:@"%@", [[self.dataSource objectAtIndex:indexPath.row] objectForKey:@"stis"]];
    mdv.str2=[NSString stringWithFormat:@"%@", [[self.dataSource objectAtIndex:indexPath.row] objectForKey:@"balancenumber"]];
    mdv.str3=[NSString stringWithFormat:@"%@", [[self.dataSource objectAtIndex:indexPath.row] objectForKey:@"withdrawStatus"]];
    mdv.str4=[NSString stringWithFormat:@"%@", [[self.dataSource objectAtIndex:indexPath.row] objectForKey:@"createTime"]];
